...
{	
	...
	function scan($nntp, $groupArr, $first, $last, $type='update')
	{
		...
		{	
			...
			{
				if ($insBinariesStmt = $db->mysqliPrepare("INSERT INTO binaries (name, fromname, date, xref, totalparts, groupID, binaryhash, dateadded) VALUES (?, ?, FROM_UNIXTIME(?), ?, ?, ?, ?, now())"))
					$insBinariesStmt->bind_param('sssssds', $pName, $pFromName, $pDate, $pXref, $pTotalParts, $pGroupID, $pBinaryHash);					
				else
					die("couldn't prepare binaries insert statement!");
				
				if ($insPartsStmt = $db->mysqliPrepare("INSERT IGNORE INTO parts (binaryID, messageID, number, partnumber, size, dateadded) VALUES (?, ?, ?, ?, ?, now())"))
					$insPartsStmt->bind_param('dssss', $pBinaryID, $pMessageID, $pNumber, $pPartNumber, $pSize);
				else
				...
				foreach($this->message AS $subject => $data)
				{
					if(isset($data['Parts']) && count($data['Parts']) > 0 && $subject != '')
					{
						...
						if(!$res)
						{
							...
							$pDate = $data['Date']; 
							$pXref = $data['Xref'];
							$pTotalParts = $data['MaxParts']; 
							$pGroupID = $groupArr['ID'];
							$pBinaryHash = $binaryHash;
							
							$insBinariesStmt->execute();
							$binaryID = $insBinariesStmt->insert_id;
							
							$count++;
							if ($count%100==0) echo "$count bin adds...\n";
						}
						else
						{
							$binaryID = $res["ID"];
							$updatecount++;
							if ($updatecount%100==0) echo "$updatecount bin updates...\n";
						}
				...
				if (($count >= 100) || ($updatecount >= 100)) { echo $n; } //line break for bin adds output
				
				$insBinariesStmt->close(); 
				$insPartsStmt->close(); 
				$db->commit(false);				
			}
	...
	private function addMissingParts($numbers, $groupID) 
	{
		...
		foreach($numbers as $number)
		{
			...
			if ($inscount % 25 == 0)
			{
				...
				$addedcnt = $addedcnt + $db->queryInsert($insertStr, false);
				$insertStr = "INSERT INTO partrepair (numberID, groupID) VALUES ";
				$added = false;
			}
		}
